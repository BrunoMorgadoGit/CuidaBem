...
export class HealthService {
  ...
  private analyzeSkinObservation(data: SkinObservationData): AnalysisResult {
    ...
    if (data.imageBase64) {
      riskScore += 2;
      findings.push('🤖 Análise de IA (Preditiva): Padrão visual sugere inflamação ou leve hiperemia na região capturada.');
      recommendations.push('Acompanhar evolução nas próximas 24 horas usando captura de imagem.');
    }
  ...
  private analyzeExcretionObservation(data: ExcretionObservationData): AnalysisResult {
    ...
    if (data.imageBase64) {
      riskScore += 1;
      findings.push('🤖 Análise de IA (Preditiva): A coloração e consistência na imagem divergem do padrão saudável esperado.');
      recommendations.push('Aumentar hidratação e reavaliar na próxima ocorrência.');
    }
